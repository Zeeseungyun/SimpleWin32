...
namespace impl {
	...
	std::enable_if_t<yjj::impl::is_all_random_arg<Arg1T, Arg2T>::value, promotion_t<Arg1T, Arg2T>>
		...
		typename yjj::impl::uniform_distiribution_selector<promotion_t>::type dist(first_, last_);
		return dist(yjj::impl::get_engine());
	}
	
	template<typename T = int32>
	std::enable_if_t<yjj::impl::is_random_arg<T>::value, T>
		rand(T end = std::numeric_limits<T>::max()) noexcept {
		typename yjj::impl::uniform_distiribution_selector<T>::type dist(end < 0 ? end : 0, end < 0 ? 0 : end);
		return dist(yjj::impl::get_engine());
	}
