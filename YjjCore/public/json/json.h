...
namespace yjj {
	...
	std::basic_string<char, TraitsT, AllocT> 
		...
		std::string temp = j;
		return yjj::to_string<TraitsT, AllocT>(temp);
	}
	...
	std::basic_string<wchar_t, TraitsT, AllocT> 
		...
		std::wstring temp = j;
		return yjj::to_wstring<TraitsT, AllocT>(temp);
	}
	...
	std::basic_string<TCHAR, TraitsT, AllocT>
		...
		std::string temp = j;
		return yjj::to_tstring<TraitsT, AllocT>(temp);
	}
...
namespace nlohmann {
	template<>
	struct adl_serializer<yjj::tstring> {
		static void from_json(const json& j, yjj::tstring& str) {
			str = yjj::to_tstring(j);
		}

		static void to_json(json& j, const yjj::tstring& str) {
			j = yjj::to_string<std::char_traits<char>, std::allocator<char>>(str);
		}
	};
}
