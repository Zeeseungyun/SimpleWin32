...
namespace impl {
	...
	private:
		default_logger() {
			...
			tstring log_file_path = yjj::file::paths::log_dir() + TEXT("log_") + tstrmanip::cur_time_to_str(TEXT("%y_%m_%d_%H_%M_%S")) + TEXT(".txt");
	...
	const TCHAR* log::verbose_to_raw_str(verbose_type  vb) {
		switch (vb)
		{
		case yjj::log::verbose_type::normal:
			return TEXT("normal");
			break;
		case yjj::log::verbose_type::warning:
			return TEXT("warning");
			break;
		case yjj::log::verbose_type::fatal:
			return TEXT("fatal");
			break;
		default:
			break;
