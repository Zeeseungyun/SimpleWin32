...
#include "../../YjjCore/public/shape/intersect.h"
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_WINDOWPOSCHANGED:
	{
		...
		app_config.window_size = to_yjj(rc).size();

		GetClientRect(app_inst->window_handle<HWND>(), &rc);
		app_config.client_size = to_yjj(rc).size();
...
void yjj::application::change_window_size(const math::vec2i& new_window_size) noexcept {
	if (is_started()) {
...
void yjj::application::change_window_position(const math::vec2i& new_window_position) noexcept {
	if (is_started()) {
