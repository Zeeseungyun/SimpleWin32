...
namespace yjj {
	...
	void bullet::load_image() {
		frame_image::get().load_frame_image(coords_bullet_[player_bullet_straight_size]
			, TEXT("assets/unit_bullet.bmp"), (int)obj_type::player_bullet_straight);
		frame_image::get().load_frame_image(coords_bullet_[monster_bullet_straight_size]
			, TEXT("assets/monster_bullet_straight.bmp"), (int)obj_type::monster_bullet_straight);
		frame_image::get().load_frame_image(coords_bullet_[monster_bullet_circle_size]
			, TEXT("assets/monster_bullet_circle.bmp"), (int)obj_type::monster_bullet_circle);
		frame_image::get().load_frame_image(coords_bullet_[monster_bullet_arround_size]
			, TEXT("assets/monster_bullet_arround.bmp"), (int)obj_type::monster_bullet_arround);
		frame_image::get().load_frame_image(coords_bullet_[monster_bullet_wave_size]
			, TEXT("assets/monster_bullet_wave.bmp"), (int)obj_type::monster_bullet_wave);


		//회전
		frame_image::get().load_plg_image(coords_bullet_[monster_bullet_homing_size]
		...
		for (float degree = 0.0f; degree != 360; degree++) {
			...
			point.x =
				degree < 1.0f ?
				coords_bullet_[monster_bullet_homing_size].x / 2 :
				coords_bullet_[monster_bullet_homing_size].x / 2 + degree * (coords_bullet_[monster_bullet_homing_size].x + coords_bullet_[monster_bullet_homing_size].x / 2);

			point.y = static_cast<float>(coords_bullet_[monster_bullet_homing_size].y / 2);
	...
	void bullet::init() {
		projectile::init();
		set_atk(1);
		if (get_obj_type() == (int)obj_type::player_bullet_straight) {
			set_now_pos_and_body(coords_bullet_[destroy_bullet_player_zone]);
		}
		else {
			set_now_pos_and_body(coords_bullet_[destroy_bullet_monster_zone]);
		}
	...
	void bullet::spawn_from(const int obj_type, const shape::circlef& obj_body) {
		...
		if (obj_type == (int)obj_type::player_straight) {
			set_size(coords_bullet_[player_bullet_straight_size]);
			set_frame_size(coords_bullet_[player_bullet_frame]);
		...
		// 총알 수 표기
		if (get_subj_type() == (int)obj_type::player_straight) {
			game_var::get().add_bullet_player(1);
		}
		else {
			game_var::get().add_bullet_monster(1);
		}
	}
