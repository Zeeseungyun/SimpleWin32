...
namespace yjj {
	...
	void stage::on_app_started() {
		...
		// 사운드
		//sound::get().play_bgm(TEXT("assets/bgm/bgm.wav"));
		PlaySound(TEXT("assets/bgm/bgm.wav"), nullptr, SND_ASYNC);
	...
	void stage::tick(float delta_time) {
		...
		*/
		...
		//플레이어 사망판단
		if (get_players()[0]->get_hp() == 0
			|| !(get_players()[0]->in_screen())) {

			if (get_players()[0]->get_state() != (int)obj_state::god) {
		...
		for (auto& bullet_player_obj : get_players()[0]->get_bullets()) {
			if (bullet_player_obj->get_state() != (int)obj_state::die) {
				if (bullet_player_obj->get_hp() == 0
					|| !(bullet_player_obj->in_screen())) {
		...
		for (auto& mon_obj : get_monsters()) {
			...
			for (auto& bullet_monster_obj : mon_obj->get_bullets()) {
				...
				//적 뷸렛 사망판단
				if (bullet_monster_obj->get_state() != (int)obj_state::die) {
		...
		if (key_state::is_down(keys::R)) {
			...
			set_game_time(0.0f);
			game_var::get().init_bullet_monster();
			game_var::get().init_bullet_player();
		}
	}
	...
	void stage::render(win32gdi::device_context_base& dest_dc, const float g_fps) {
		...
		HBRUSH myBrush = (HBRUSH)CreateSolidBrush(RGB(20, 20, 20));
		HBRUSH oldBrush = (HBRUSH)SelectObject((HDC)back_buffer_.handle_dc(), myBrush);
		Rectangle((HDC)back_buffer_.handle_dc(), 700, 0, 1200, 1100);
		SelectObject((HDC)back_buffer_.handle_dc(), oldBrush);
		DeleteObject(myBrush);


		//shape::rectf box(700, 0, 1200, 1100);
		//back_buffer_.rectangle(box);
		shape::rectf box_info(750, 580, 980, 800);
		back_buffer_.rectangle(box_info);
		back_buffer_.print_text({ 800, 600 }, TEXT("FPS: "));
		back_buffer_.print_text({ 880, 600 }, to_tstring(to_wstring(g_fps)));
		back_buffer_.print_text({ 800, 620 }, TEXT("시간: "));
		back_buffer_.print_text({ 880, 620 }, to_tstring(to_wstring(static_cast<int>(get_game_time()))));
		back_buffer_.print_text({ 800, 640 }, TEXT("점수: "));
		back_buffer_.print_text({ 880, 640 }, to_tstring(to_wstring(get_players()[0]->get_my_score())));
		back_buffer_.print_text({ 800, 660 }, TEXT("최고점수: "));
		back_buffer_.print_text({ 880, 660 }, to_tstring(to_wstring(get_players()[0]->get_high_score())));
		back_buffer_.print_text({ 800, 700 }, TEXT("충돌범위:    Tab"));
		back_buffer_.print_text({ 800, 720 }, TEXT("리스폰:        R"));
		back_buffer_.print_text({ 800, 740 }, TEXT("(치트)무적:    T"));
		back_buffer_.print_text({ 800, 760 }, TEXT("총알 개수: "));
		back_buffer_.print_text({ 880, 760 }, to_tstring(to_wstring(game_var::get().bullet_monster_total() + game_var::get().bullet_player_total() )));
