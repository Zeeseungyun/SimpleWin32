...
namespace yjj {
	...
	void player::load_image() {
		frame_image::get().load_frame_image(coords_player_[player_size]
			, TEXT("assets/player.bmp"), (int)obj_type::player_straight);
	}
	
	void player::init() {
		plane::init();
		set_now_pos_and_body(coords_player_[destroy_player_zone]);
	...
	void player::spawn() {
		...
		set_now_pos_and_body(
			{ coords_player_[player_default_pos].x + get_body().radius,
			coords_player_[player_default_pos].y + get_body().radius }
		);
		set_delay(0.2f);
	}
	...
	void player::move(const float delta_time) {
		...
		//키 입력 이벤트
		if (key_state::is_toggle_on(keys::T)) {
			set_state((int)obj_state::god);
		}
	...
	void player::shoot(const float delta_time) {
		...
		if (key_state::is_down(keys::space)) {
			if (in_screen() &&
				...
				get_state() == (int)obj_state::god)) {
				plane::shoot_per_delay(frame);
			}
		}
		/*
		if (key_state::is_pressed(keys::space)) {
			if (in_screen() &&
				(get_state() == (int)obj_state::idle ||
					get_state() == (int)obj_state::god)) {
				//sound::get().play_sound_effect(TEXT("assets/sound_effect/Laser_Shoot5.wav"));
			}
		}
		*/
		...
		if (in_screen()) {
			if (get_state() == (int)obj_state::idle
				|| get_state() == (int)obj_state::god) {
					...
					if (get_state() == (int)obj_state::god) {
						dest_dc.print_text(get_body().origin, TEXT("무적"));
					}
			}
		}
