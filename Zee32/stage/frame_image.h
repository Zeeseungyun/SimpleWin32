...
namespace zee {
	...
	public:
		...
		void render_transparent_destdc_to_backbuffer(win32gdi::device_context_dynamic& dest_dc, const math::vec2i& dest_pos, const int i);
		

		void load_plg_image(const math::vec2i& frame_size, const TCHAR* filename, const int i);
		void render_plg(win32gdi::device_context_dynamic& dest_dc, const math::vec2f& point, const float angle, const int i);
		void render_plg_transparent(win32gdi::device_context_dynamic& dest_dc, const math::vec2i& dest_pos, const math::vec2i& src_pos, const int i);
	...
	private:
		...
		const int image_monster_arround_num_ = 360;
	};
}